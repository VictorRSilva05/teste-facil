...
public sealed class RepositorioQuestaoEmOrm : TestFixture
{
    ...
    public void Deve_Selecionar_Questoes_Corretamente()
    {
        ...
        //Act
        var registroRecebidos = repositorioQuestao?.SelecionarRegistros();

        //Assert
        var registroEsperadosOrdenados = registrosEsperados
            .OrderBy(x => x.Enunciado).ToList();

        CollectionAssert.AreEqual(registroEsperadosOrdenados, registroRecebidos);
    }
}
